...
4. **人物标签（仅无标签者）**：删除 / 合并 / 改名必须**通盘考虑整个人物库**。其中 **tagAssignments 只对**输入里 **relationshipsSample 为空** 的 id 补 1～3 个简洁统一标签（如：家人、朋友、同学、同事、恋人、自己、其他）。已有标签的人**不要**写进 tagAssignments；若误写，系统会自动忽略该条，不影响其余整理。
...
export const PERSON_ORGANIZE_USER_PROMPT = (personsJson: string) =>
  `请整理以下人物库条目（JSON 数组，每项含 id、name、appearances、dreamCount、relationshipsSample）：\n\n${personsJson}`;
